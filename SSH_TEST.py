...
def login(hostip, username, password):
    """登录 H3C 设备，返回 (连接对象/None, 失败原因(成功为None))"""
    try:
        device = {
...
def _print_usage():
    print("用法: python SSH_test.py '{\"hostip\":\"192.168.30.7\",\"username\":\"admin\",\"password\":\"admin\"}'")
    print("说明: 仅接受一个 JSON 字符串参数; 现在仅支持单一 hostip。")
...
if __name__ == "__main__":
    ...
    if not ip:
        ...
        write_log.write_log(
            ...
            system_info={},
            file_suffix="ssh_test",
            error_message={"General": "未提供 hostip"}
        )
        sys.exit(4)
    if ',' in ip:
        ...
        write_log.write_log(
            ...
            system_info={},
            file_suffix="ssh_test",
            error_message={"General": "检测到多个IP，当前仅支持单设备"}
        )
        sys.exit(4)
    print(f"\n==== 尝试登录设备 {ip} ====")
    if not check_ip(ip):
